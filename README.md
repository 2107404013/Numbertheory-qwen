...
Stage 2 会从公开奥赛级数学数据中构造固定的正式数论评测集。第一版只筛选 Number Theory 题，目标规模为 200 道。
...
当前阶段：Stage 2 - Build Formal Number Theory Evaluation Set。当前只实现正式数论评测集构造与审计，不进入 Stage 3。
