...
如果后续能够读取 GitHub raw 的 `omni_math_rule.jsonl`，则优先使用 rule-based subset 构造正式评测集；否则从 `KbsdJames/Omni-MATH` 中筛选纯文本、短答案、Number Theory 题。正式评测集规模第一版固定为 200 道题。训练集第一版使用 5k 数论样本，确认提升后再扩展到 10k。
...
当前阶段：Stage 1 - Scoring Protocol and Dataset Inspection。当前仍在修正公开数据集字段检查逻辑，不进入 Stage 2。
